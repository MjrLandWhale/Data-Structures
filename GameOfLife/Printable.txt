 ...
 * Mitchell Brandone
 * 10-5-2015
 * Game Of Life, Formatted in accordance with Dr. Gourds...
 *  interesting choice in formatting preferences>
 ***************************************************************/
...
void ShowUsage(char* filename)
{
	...
	cout << "  -a<val>\tSet the millisecond animation interval to <val> (=0)\n";
...
void DisplayBoard(bool board[MAX_WIDTH][MAX_WIDTH], int gen, int len)
{
	...
	//New Line because the Diff gods demand it
	cout << "\n";
}

//calculates and updates board with next generation
		...
		for(int j = 1; j < len - 1; j++)
		{
			...
			for(int k = -1; k <= 1; k++)
			{
				for(int l = -1; l <= 1; l++)
				{
					...
					//True if selected neighbor is alive, false otherwise
					if(boardCopy[i + k][j + l])
						//Increment numAlive when neighbor is alive
						numAlive++;
				}
			}
...
// animate specified number of milliseconds
